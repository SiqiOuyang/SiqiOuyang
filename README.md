# 👋 Hi there, I'm 欧阳思琦

## 🎓 About Me
- 16岁考上湘潭大学（双一流） 现就读于计算机学院网络空间安全专业
- 对人工智能安全、机器学习、进化计算感兴趣
- 正在探索AI在网络安全中的应用

## 🔭 I'm currently working on
- 学习机器学习基础（吴恩达课程、sklearn实践）
- 数据结构与算法（LeetCode基础题，每天2-3道）
- 自学Python和PyTorch，准备做一个小项目

## 🌱 I'm currently learning
- Python / PyTorch
- 机器学习 / 深度学习入门
- 计算机基础（数据结构、离散数学）

## 💡 My learning journey
- 喜欢动手实践，正在尝试用代码解决实际问题
- 每周整理学习笔记，分享在GitHub上

## 📫 How to reach me
- Email: 202505180522@smail.xtu.edu.cn
- GitHub: [@SiqiOuyang](https://github.com/SiqiOuyang)

## 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=SiqiOuyang&show_icons=true&theme=tokyonight)

## 📝 Latest activity
- 正在整理数据结构“三遍法”笔记
- 学习离散数学题型总结
- 计划近期开始第一个PyTorch小项目
- 开始用git管理代码了，加油！！我可以
- 准备期末复习  成为学工办的一员  敲了最小生成树 二分图
